...
resource "aws_internet_gateway" "ilg-edu_itg" {
  vpc_id = aws_vpc.ilg-edu.id
...
resource "aws_route_table" "public_routes" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.ilg-edu_itg.id
  }
}
...
resource "aws_nat_gateway" "ilg-edu_nat" {
...
resource "aws_route_table" "private_routes" {
  count = length(var.private_subnets)
  vpc_id     = aws_vpc.ilg-edu.id
  depends_on = [aws_nat_gateway.ilg-edu_nat]
